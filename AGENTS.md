...
이 파일은 이 저장소에서 작업하는 Agent를 위한 가이드입니다.
...
- **axios** — HTTP 클라이언트 (`src/api/client-fetcher.ts` 단일 인스턴스)
- **Zustand** — 클라이언트 전역 상태
- **react-hook-form** — 폼
- **Motion** — 애니메이션 라이브러리
...
- 도메인별 API 함수·쿼리 키는 `src/api/<domain>.ts`, 이를 TanStack Query로 감싼 커스텀 훅은 `src/hooks/<domain>.ts`에 둔다(둘 다 도메인당 단일 파일). 컴포넌트는 `fetch`/`axios`/`useQuery`를 직접 호출하지 않고 **커스텀 훅만** 사용한다 (HTTP 클라이언트 단일 인스턴스는 `src/api/client-fetcher.ts`)
- 환경 변수
...
> 구현 패턴·쿼리 키 팩토리·커스텀 훅 작성법·무한 스크롤 등 상세는 **`.claude/skills/state-management/SKILL.md` 스킬**을 로드해 참고한다.
...
- **신규 작업**: 이슈가 없으면 **먼저 `superpowers:brainstorming`으로 요구사항을 정리**한다. 사용자가 설계를 승인하면, 그 결과를 알맞은 이슈 템플릿에 채워 `gh issue create`로 등록한 뒤 그 이슈로 구현한다
...
> 브랜치 네이밍·커밋 타입·PR 형식 등 상세는 **`.claude/skills/git-workflow/SKILL.md` 스킬**을 로드해 참고한다.

- main/dev 직접 푸시 금지, 모든 변경은 PR을 거친다. 이슈 없이 작업 시작 금지.

## Figma 연동

> fileKey·MCP 호출 절차·디자인 토큰 매핑 등 상세는 **`.claude/skills/figma-integration/SKILL.md` 스킬**을 로드해 참고한다.
