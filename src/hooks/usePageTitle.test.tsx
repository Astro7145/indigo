...
describe('usePageTitle', () => {
  ...
  it('/posts 경로면 posts 카탈로그의 title 키로 타이틀을 만든다', () => {
    expect(titleFor('/posts')).toBe('posts.title');
  });

  it('/posts/write 경로면 posts 카탈로그의 form.createTitle 키를 사용한다', () => {
    expect(titleFor('/posts/write')).toBe('posts.form.createTitle');
  });

  it('/posts/123/edit 경로면 posts 카탈로그의 form.editTitle 키를 사용한다', () => {
    expect(titleFor('/posts/123/edit')).toBe('posts.form.editTitle');
  });
});
