...
it('위에 다른 모달이 열려 비활성화돼도(active=false) 배경 트리거로 포커스를 되돌리지 않는다', () => {
  // Modal A 위에 Modal/BottomSheet B가 열려 A가 active=false가 될 때,
  // A가 배경 페이지로 포커스를 강제 복귀시키면 B 뒤로 포커스가 새는 a11y 버그가 발생한다.
  function Harness({ open, active }: { open: boolean; active: boolean }) {
    return (
      <>
        <button data-testid="trigger">열기</button>
        <Modal open={open} active={active} onClose={() => {}}>
          <button data-testid="inside">내부</button>
        </Modal>
      </>
    );
  }
  const { rerender } = render(<Harness open={false} active />);
  screen.getByTestId('trigger').focus();
  rerender(<Harness open active />);
  expect(screen.getByTestId('inside')).toHaveFocus();
  rerender(<Harness open active={false} />);
...
it('active=false이면 열려도 내부 첫 요소로 포커스를 가져가지 않는다', () => {
  render(
    <Modal open active={false} onClose={() => {}}>
      <button data-testid="inside">내부</button>
    </Modal>,
  );
...
it('active=false이면 Tab 포커스를 모달 내부에 가두지 않는다', () => {
  render(
    <Modal open active={false} onClose={() => {}}>
      <button data-testid="first">first</button>
      <button data-testid="last">last</button>
...
it('zIndex를 지정하면 백드롭에 inline z-index로 적용한다', () => {
  render(
    <Modal open zIndex={70} onClose={() => {}}>
      <p>내용</p>
    </Modal>,
  );
  expect(screen.getByTestId('modal-backdrop')).toHaveStyle({ zIndex: 70 });
});

it('zIndex 미지정 시 기본 z-50 클래스를 유지한다', () => {
  render(
    <Modal open onClose={() => {}}>
      <p>내용</p>
    </Modal>,
  );
  expect(screen.getByTestId('modal-backdrop')).toHaveClass('z-50');
});

it('scrollLock=false이면 body 스크롤을 잠그지 않는다', () => {
  render(
    <Modal open scrollLock={false} onClose={() => {}}>
      <p>내용</p>
    </Modal>,
  );
  expect(document.body.style.overflow).toBe('');
