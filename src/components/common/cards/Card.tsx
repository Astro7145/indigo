 ...
 *
 * "흰 배경 + 라운드 + 그림자 + 패딩(기본 16px)" 표면 토큰만 책임지며, 너비/높이/패딩/내부
 * 레이아웃은 호출하는 도메인 카드(PostCard, NoteCard 등)가 className으로 결정한다.
 * 기본 패딩보다 큰 카드는 className에 `p-8` 등을 넘겨 override 한다.
 *
 ...
 * @param className 추가 Tailwind 클래스 (`cn()` 병합으로 기본 패딩 등 override 가능)
 * @param ref       내부 `<div>`에 직접 부착할 ref (React 19 — `forwardRef` 미사용)
 * @param children  카드 내부 콘텐츠
 *
 * @example
 * <Card className="w-[384px] p-8" onClick={() => router.push(href)}>
 *   <h3>{post.title}</h3>
 * </Card>
 */
export default function Card({ className, children, ref, onClick, role, tabIndex, onKeyDown, ...props }: CardProps) {
