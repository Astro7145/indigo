...
it('onClick 없는 액션(인디케이터)은 button이 아니라 비대화형으로 렌더한다', () => {
  render(
    <TodoList title="할일">
      <TodoList.Actions>
        <TodoList.NoteAction />
  ...
  // 동작 없는 인디케이터가 포커스 잡히는 no-op <button>이면 a11y 위반 — img 역할로 표시만
  expect(screen.queryByRole('button', { name: '노트' })).not.toBeInTheDocument();
  expect(screen.getByRole('img', { name: '노트' })).toBeInTheDocument();
});
...
it('EditAction·KebabAction은 hoverOnly 아이콘 버튼이다', () => {
  ...
  expect(screen.getByRole('button', { name: '더보기 메뉴' }).className).toMatch(/group-hover/);
});
