...
it('StarAction은 아이콘 버튼이며 클릭 시 onClick을 호출한다', () => {
  ...
  render(
    <TodoItem title="할일">
      <TodoItem.Actions>
        <TodoItem.StarAction onClick={onClick} />
      </TodoItem.Actions>
    </TodoItem>,
  );
...
it('StarAction은 active prop으로 채운 별이 된다', () => {
  render(
    <TodoItem title="할일">
      <TodoItem.Actions>
        <TodoItem.StarAction onClick={() => {}} active />
      </TodoItem.Actions>
    </TodoItem>,
  );
...
it('StarAction의 active는 checked(done)와 독립이다', () => {
  ...
  render(
    <TodoItem title="할일" checked>
      <TodoItem.Actions>
        <TodoItem.StarAction onClick={() => {}} />
      </TodoItem.Actions>
    </TodoItem>,
  );
...
it('NoteAction·LinkAction은 상시 표시 아이콘 버튼이다 (hoverOnly 아님)', () => {
  render(
    <TodoItem title="할일">
      <TodoItem.Actions>
        <TodoItem.NoteAction onClick={() => {}} />
        <TodoItem.LinkAction onClick={() => {}} />
      </TodoItem.Actions>
    </TodoItem>,
  );
...
it('EditAction·KebabAction은 hoverOnly 아이콘 버튼이다', () => {
  render(
    <TodoItem title="할일">
      <TodoItem.Actions>
        <TodoItem.EditAction onClick={() => {}} hoverOnly />
...
it('액션 클릭은 행으로 버블링되지 않는다 (stopPropagation)', () => {
  ...
  render(
    <div onClick={onRowClick}>
      <TodoItem title="할일">
        <TodoItem.Actions>
          <TodoItem.StarAction onClick={onStar} />
        </TodoItem.Actions>
      </TodoItem>
    </div>,
  );
...
it('size="small"이면 본문에 text-sm를 적용한다', () => {
  render(<TodoItem title="작은 할일" size="small" />);
  expect(screen.getByText('작은 할일').className).toMatch(/text-sm/);
});

it('variant="onDark"이면 white 체크박스를 렌더한다', () => {
  const { container } = render(<TodoItem title="할일" variant="onDark" />);
...
it('컨텍스트 밖에서 서브컴포넌트를 쓰면 에러를 던진다', () => {
  ...
  expect(() => render(<TodoItem.StarAction onClick={() => {}} />)).toThrow();
  spy.mockRestore();
});

it('행을 클릭하면 onClick을 호출한다', () => {
  const onClick = jest.fn();
  render(<TodoItem title="자바스크립트 듣기" onClick={onClick} />);
...
it('체크박스를 클릭하면 토글만 되고 행 onClick은 호출되지 않는다', () => {
  const onClick = jest.fn();
  const onCheckedChange = jest.fn();
  render(<TodoItem title="할일" onClick={onClick} onCheckedChange={onCheckedChange} />);
  fireEvent.click(screen.getByRole('checkbox'));
...
it('우측 액션 버튼을 클릭하면 행 onClick은 호출되지 않는다', () => {
  ...
  const onStar = jest.fn();
  render(
    <TodoItem title="할일" onClick={onClick}>
      <TodoItem.Actions>
        <TodoItem.StarAction onClick={onStar} />
      </TodoItem.Actions>
    </TodoItem>,
