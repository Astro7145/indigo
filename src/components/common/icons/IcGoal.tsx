...
export function IcGoal({ size = 'sm', bgShape = 'square', className, ...rest }: IcGoalProps) {
  ...
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn(size === 'lg' ? 'size-[180px]' : 'size-8', className)} {...rest}>
      {bgShape === 'circle' ? (
        <circle cx="20" cy="20" r="20" fill="var(--color-indigo-300)" className="dark:fill-indigo-dark-500" />
      ) : (
        <rect width="40" height="40" rx="4" fill="var(--color-indigo-300)" className="dark:fill-indigo-dark-500" />
      )}
      <svg x="6.25" y="7.5" width="28" height="25" viewBox="0 0 19 20">
        ...
        <line
          ...
          strokeWidth="1.6"
          className="dark:stroke-indigo-dark-800"
        />
        <path
          ...
          className="dark:fill-indigo-dark-800"
        />
      </svg>
      <defs>
