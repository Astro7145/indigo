...
it('"모두 읽기" 버튼이 표시된다', () => {
  render(<NotificationPanel />)
  expect(
    screen.getByRole('button', { name: '모든 알림을 읽음으로 표시' }),
  ).toBeInTheDocument()
})
...
it('알림이 있으면 목록이 렌더링된다', () => {
  ...
  expect(
    within(list).getByRole('button', { name: READ_NOTIFICATION.message }),
  ).toBeInTheDocument()
})
...
it('hasNextPage가 true이면 "더 보기" 버튼이 표시된다', () => {
  ...
  render(<NotificationPanel />)
  expect(
    screen.getByRole('button', { name: '이전 알림 더 불러오기' }),
  ).toBeInTheDocument()
})
...
it('hasNextPage가 false이면 "더 보기" 버튼이 없다', () => {
  render(<NotificationPanel />)
  expect(
    screen.queryByRole('button', { name: '이전 알림 더 불러오기' }),
