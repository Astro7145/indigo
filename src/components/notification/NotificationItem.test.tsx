...
import NotificationItem, { type NotificationItemProps } from '@/src/components/notification/NotificationItem';
...
it('isRead가 false이면 aria-label에 "읽지 않은 알림:" 접두어가 포함된다', () => {
  renderItem({ onClick: jest.fn() });
  expect(screen.getByRole('button', { name: `읽지 않은 알림: ${BASE.message}` })).toBeInTheDocument();
});
