  ...
  return (
    <li
      ...
      className={cn(
        ...
        isInteractive &&
          'cursor-pointer hover:bg-slate-50 focus-visible:ring-2 focus-visible:outline-none',
      )}
    >
      {/* 읽지 않은 알림 인디케이터 */}
      <span
      ...
      <div className="flex min-w-0 flex-1 items-center gap-3">
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex flex-col gap-0.5 text-sm leading-5 tracking-[-0.03em]">
            <p
              className={cn(
                'font-medium wrap-break-word text-slate-700',
                !subtext && 'line-clamp-2',
              )}
            >
              {message}
            </p>
            {subtext && (
              <p className="truncate font-normal text-slate-500">{subtext}</p>
            )}
          </div>
