...
export default function TodoFormUI({
  ...
  const {
    ...
    resolver: zodResolver(todoCreateSchema(tv)),
    mode: 'onChange',
    defaultValues: {
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-2 pb-8">
          <span className="px-1 text-sm font-semibold text-slate-700 sm:text-base">{t('fields.goal')}</span>
          <Dropdown>
            <Dropdown.Trigger asChild>
              ...
              >
                {selectedGoal?.title ?? t('form.goalPlaceholder')}
                <IcChevron direction="down" className="size-5 shrink-0 text-slate-400 sm:size-6" />
              </button>
            </Dropdown.Trigger>
            <Dropdown.Menu size="full">
              <Dropdown.Item onClick={() => setValue('goalId', undefined)}>{t('form.goalNone')}</Dropdown.Item>
        ...
        {/* 태그 */}
        <div className="flex flex-col gap-2">
          <span className="px-1 text-sm font-semibold text-slate-700 sm:text-base">{t('fields.tag')}</span>
        ...
        {/* 이미지 */}
        <div className="flex flex-col gap-2">
          <span className="px-1 text-sm font-semibold text-slate-700 sm:text-base">{t('fields.image')}</span>
          <ImageInput onFileChange={setImageFile} initialUrl={fileUrl} onInitialUrlRemove={() => setFileUrl(null)} />
          <p className="text-sm font-medium text-slate-400">{t('form.imageHint')}</p>
        </div>
      </form>
