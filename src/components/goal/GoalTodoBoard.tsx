...
export default function GoalTodoBoard({ goal, className }: GoalTodoBoardProps) {
  ...
  const requestCreateClose = () => setShowCreateConfirm(true);
  ...
  return (
    <>
      ...
      */}
      ...
      {isMobile ? (
        <BottomSheet isOpen={isCreateOpen} onClose={requestCreateClose}>
          <div className="flex max-h-[90dvh] flex-col overflow-y-auto px-4 pb-4">
            <TodoCreateContainer
        ...
        <Modal open={isCreateOpen} onClose={requestCreateClose} className="p-4 sm:p-8">
          <TodoCreateContainer
            defaultGoalId={goal.id}
            onClose={() => setIsCreateOpen(false)}
            onCancel={requestCreateClose}
      ...
      >
        ...
        <Modal.Actions className="mt-2">
          <Modal.Cancel onClick={() => setShowCreateConfirm(false)}>아니오</Modal.Cancel>
          <Modal.Confirm onClick={confirmCreateClose}>예</Modal.Confirm>
        </Modal.Actions>
      </Modal>
    </>
  );
