...
// shouldRedirectToLogin 판정용 최소 AxiosError. 리다이렉트는 응답 본문 code === 'TOKEN_INVALID'로 트리거된다.
function err(code: string | undefined, url: string): AxiosError {
  return { isAxiosError: true, config: { url }, response: { data: { code } } } as unknown as AxiosError;
}
...
it('TOKEN_INVALID 코드가 아니면 리다이렉트하지 않는다', () => {
  window.history.pushState({}, '', '/dashboard');
  expect(shouldRedirectToLogin(err('NOT_FOUND', '/todos/1'))).toBe(false);
});
