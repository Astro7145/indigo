...
it('login은 /iauth/login으로 POST하고 { user }를 반환한다', async () => {
...
it('signup은 /iauth/signup으로 POST하고 { user }를 반환한다', async () => {
...
it('refresh는 body 없이 /iauth/refresh로 POST하고 void를 반환한다', async () => {
  mocked.post.mockResolvedValue({ data: undefined } as never);
  const r = await refresh();
  expect(mocked.post).toHaveBeenCalledWith('/iauth/refresh');
  expect(r).toBeUndefined();
});
it('logout은 body 없이 /iauth/logout으로 POST하고 void를 반환한다', async () => {
  mocked.post.mockResolvedValue({ data: undefined } as never);
